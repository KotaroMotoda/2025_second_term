...
### 仮想記憶とは
- プログラムが使うアドレス空間（仮想アドレス）と、実際の物理メモリ上のアドレス（物理アドレス）を分離する仕組み。
- 各プロセスは「自分専用の大きなメモリ空間」を持っているように見える。
- 実際には、OSとハードウェア（MMU: メモリ管理ユニット）が、仮想アドレスと物理アドレスの対応（アドレス変換）を管理している。

### 主な目的
- **メモリ保護**：プロセスごとに独立したアドレス空間を持たせ、他プロセスのメモリ領域への不正アクセスを防ぐ。
- **アドレス空間の拡張**：物理メモリより大きな仮想メモリ空間を使える（ページングによるスワップ）。
- **メモリ管理の柔軟化**：プログラムの配置やメモリ割り当てを柔軟にできる。

### 仕組み
- 仮想アドレス空間は「ページ」と呼ばれる固定長の単位（例：4KB）で分割される。
- ページごとに「ページテーブル」で仮想アドレス→物理アドレスの対応を管理。
- 必要なページだけを物理メモリにロードし、足りない場合はディスク（スワップ領域）に退避。
- ページフォールト：アクセスしたページが物理メモリ上にない場合、OSがディスクから読み込む。

### メリット
- プログラム同士の干渉を防げる
- メモリ不足時に一部をディスクに退避できる（スワップ）
- プログラムの再配置や共有ライブラリの利用が容易

### デメリット
- ページフォールト発生時は大きな遅延が生じる
- ページテーブル管理のためのオーバーヘッド

### まとめ
仮想記憶は、現代OSのメモリ管理の基盤となる仕組みであり、セキュリティ・柔軟性・効率性を高める重要な役割を担っている。
````
...
| 問題番号 | 問題文（要約）                    | 選択肢 a                   | 選択肢 b                      | 選択肢 c                         | 選択肢 d                      | 正答 |
| :--: | :------------------------- | :---------------------- | :------------------------- | :---------------------------- | :------------------------- | :- |
