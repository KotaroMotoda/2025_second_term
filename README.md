...
| 並列性レベル         | 主な実装例                                      | 説明・特徴                                 |
|----------------------|------------------------------------------------|--------------------------------------------|
| プロセッサレベル     | マルチコア、SMP/NUMA、クラスタ、グリッド        | 複数のコア/ノード/マシンで同時に命令実行   |
| スレッドレベル       | SMT（同時マルチスレッド）、粗粒度/細粒度MT      | 1コア内で複数スレッドが同時に命令発行      |
| 命令レベル（ILP）    | パイプライン、スーパースカラ、VLIW              | 1スレッド内で複数命令を同時/段階的に実行   |
| データレベル（DLP）  | SIMDプロセッサ、ベクトルプロセッサ、GPU         | 1命令で複数データを同時に処理             |
| スカラ               | スカラアーキテクチャ                            | 1命令1データ、逐次実行が基本               |
| ドメイン固有         | ドメイン固有アーキテクチャ                      | 特定用途に特化した並列処理                 |
