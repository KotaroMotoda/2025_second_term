...
> **ベクトルアーキテクチャとの違い**
> - SIMDは「固定長」単位で同時処理。命令ごとに幅が決まる。
> - ベクトルアーキテクチャは「可変長」の長いデータ列全体を1命令で指定でき、ハードウェアが自動で分割・繰り返し処理する。
> - ベクトルアーキテクチャでは、プログラムはベクトル長を意識せずに記述できる。
> - どちらも「データ並列」だが、命令の粒度や自動化の度合いが異なる。
