...
|---|---|---|---|
| **データ並列**<br/>・同じ処理を大量のデータ要素に対して同時に適用<br/>・行列演算、GPUによる画像フィルタ処理 | **SIMD**<br/>・単一命令、複数データ<br/>・ベクトル(一次元配列)を操作 | **SIMDプロセッサ**<br/>・単一命令で複数データを同時処理<br/>**ベクトルプロセッサ**<br/>・ベクトル命令で長いデータ列を同時処理 | ・行列演算<br/>・GPUによる画像フィルタ処理 |
...
| 並列性レベル         | 主な実装例                                      | 説明・特徴                                 |
|----------------------|------------------------------------------------|--------------------------------------------|
| プロセッサレベル     | ・マルチコア：1つのチップに複数の独立したCPUコア<br/>・SMP/NUMA：複数CPU間で共有/分散メモリ<br/>・クラスタ：独立したマシンをネットワーク接続<br/>・グリッド：地理的に分散したマシン群 | ・マルチコア：各コアが独立して命令実行、共有キャッシュで連携<br/>・SMP：均等なメモリアクセス時間<br/>・NUMA：メモリアクセス時間が不均等<br/>・クラスタ/グリッド：メッセージパッシングで通信 |
| スレッドレベル       | ・SMT：1コアで複数スレッドを同時実行<br/>・粗粒度MT：長時間ストール時に切替<br/>・細粒度MT：命令ごとに切替 | ・SMT：遊休資源を他スレッドが利用し効率化<br/>・粗粒度MT：メモリ待ちなど長期ストール時のみ切替<br/>・細粒度MT：毎サイクルのようにきめ細かく切替 |
| 命令レベル（ILP）    | ・パイプライン：命令を段階的に処理<br/>・スーパースカラ：複数の命令を同時発行<br/>・VLIW：長い命令語で明示的に並列指定 | ・パイプライン：各段階を並列実行でスループット向上<br/>・スーパースカラ：実行時に依存関係を解析して並列化<br/>・VLIW：コンパイル時に並列性を抽出 |
| データレベル（DLP）  | ・SIMDプロセッサ：固定長ベクトル処理<br/>・ベクトルプロセッサ：可変長ベクトル処理<br/>・GPU：多数の演算コアで大規模並列処理 | ・SIMD：128/256/512bit等の固定長一括処理<br/>・ベクトル：長いデータ列を自動分割して処理<br/>・GPU：数千コアで画像/行列計算等を超並列処理 |
| スカラ               | ・スカラアーキテクチャ：基本的な1命令1データ処理 | ・命令とデータが1:1で対応<br/>・パイプライン/キャッシュで高速化<br/>・ハザード検出/回避が必要 |
| ドメイン固有         | ・特定用途向け専用プロセッサ<br/>・AI/画像/信号処理等に特化 | ・汎用性は低いが特定処理で高性能<br/>・電力効率が良い<br/>・専用コンパイラ/ライブラリが必要 |
