...
- **GPU**: 多数の演算ユニットでデータ並列処理. マルチスレッディングを行うことでレイテンシを隠す.
- **SMT（同時マルチスレッド）**: 1コア内の**複数スレッド**が同時に命令を発行
...
- **粗粒度マルチスレッド**: 長時間のストールが発生したと時にスレッド切替
- **細粒度マルチスレッド**: 命令ごとにスレッド切替
- **パイプライン**: 命令を段階的に重ねて実行

### 命令レベル並列（ILP） `[単位: 命令]`
- **スーパースカラ**: **1つの**スレッドの中から複数命令を同時発行
- **VLIW**: 複数命令を1命令語で同時実行
...
基本的に「一次元のデータ配列（ベクトル）」を対象に、1命令で複数要素を同時に処理する。
ただし、SIMDは「固定長」のデータ幅（例: 128bit, 256bit, 512bitなど）で同時処理するため、
配列が長い場合は複数回の命令発行が必要。
プログラム側でSIMD命令を明示的に使うことが多い。
（例：x86のSSE/AVX、ARMのNEONなど）

> **ベクトルアーキテクチャとの違い**
> - **共通点**: どちらもソフトウェアの「データ並列性」を「SIMDモデル」で実現する技術
> - **SIMDプロセッサ**: 「固定長」単位で同時処理。命令ごとに幅が決まる。プログラマーがデータ長を意識して分割
> - **ベクトルプロセッサ**: 「可変長」の長いデータ列全体を1命令で指定でき、ハードウェアが自動で分割・繰り返し処理する
> - **プログラマー体験**: ベクトルアーキテクチャでは、プログラムはベクトル長を意識せずに記述できる
> - **実装**: どちらも「データ並列」だが、**自動化の度合い**や**命令の粒度**が異なる

#### ベクトルアーキテクチャ
ベクトル命令で複数データをまとめて処理。1命令で「可変長」の長いデータ列（ベクトル全体）を指定でき、ハードウェアが自動的に分割して複数の演算ユニットで繰り返し処理する（ストリーミング）。
データハザードの確認はベクトル単位で効率的に行われるため、スカラ型よりもオーバーヘッドが小さい。メモリアクセスも連続アクセスが多く、レイテンシ隠蔽が可能。
（例：Crayベクトルマシン、NEC SXシリーズなど）

> **SIMDプロセッサとの共通点**: ソフトウェアの「データ並列性」を「SIMDモデル」で実現
